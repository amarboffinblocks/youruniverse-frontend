...
const Page = () => {
    const [characterPreview, setCharacterPreview] = useState(false);
    return (
        <div className="flex-1 flex  relative overflow-hidden min-h-0 ">
            <div className="flex-1 flex   min-h-0  ">
                <Container className={cn('h-full w-full min-h-0   ', characterPreview && "mr-0 max-w-6xl ")}>
                    <Chats setPreviewModel={setCharacterPreview} />
                </Container>
            ...
            </div>
            {
                characterPreview && (
                    <div className='max-w-sm w-full   relative   '>
                        <Button size={"icon"} className='  absolute  -left-8   top-2 z-50' onClick={() => setCharacterPreview(false)}>
                            <X />
                        </Button>
